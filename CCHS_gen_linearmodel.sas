...
FILENAME REFFILE 
	"C:/Users/namakuto/Documents/Data Management/Assignment 4/cchs.csv";
PROC IMPORT DATAFILE=REFFILE DBMS=CSV OUT=smoking.data1;
	GETNAMES=YES;
RUN;
...
data smoking.data1;
	set smoking.data1;
	drop var VAR14; run;
proc contents data=smoking.data1;
run;
proc freq data=smoking.data1;
run;

**** Renaming variables;
data smoking.datalab;
	set smoking.data1;
	rename ALC_015=Alcohol CCC_095=Diabetes DEN_030=DentalVisits 
		...
		EHG2DVR3=Education FDCDVAVD=Food HWTDGBCC=BMI INCDGPER=Income OHT_005=OralH 
		SMKDGSTP=StopSmoking;
run;
...
data smoking.dataoralh;
	set smoking.datalab;
	array x education bmi diabetes oralh toothbrushing dentalvisits food 
		stopsmoking drugs;

	do i=1 to dim(x);

		if x[i]=>6 and x[i]<=9 then
			x[i]=.;
	end;
	drop i;
	...
	if oralh<=3 and oralh>=1 then
		oralh=1;
	else if oralh>3 and oralh<6 then
		oralh=0;
run;
...
OralH oralh_l. StopSmoking smokestop_l.;
run;

/*###############################################
Descriptive analyses
################################################*/
proc freq data=smoking.dataoralh;
...
data modsmoke;
	set smoking.dataoralh;
	if stopsmoking=. then
		stopsmoking2="Non-former";
	else
		stopsmoking2="former"; run;
...
proc freq data=modsmoke;
	tables oralh*(age income alcohol diabetes dentalvisits toothbrushing sex drugs 
		education food bmi stopsmoking) /nopercent nocol nofreq nocum missing chisq;
run;

/*###############################################
Log-Binomial Model
################################################*/
*** Need to make dummies... <--- NO, not needed due to ability to set reference in GLM;

/* data smoking.dummies;
	set smoking.dataoralh;

	if stopsmoking=0 then
		smk0=1;
	else
		smk0=0;
	if stopsmoking=1 then
		smk1=1;
	else
		smk1=0;
	if stopsmoking=2 then
		smk2=1;
	else
		smk2=0;
	if stopsmoking=3 then
		smk3=1;
	else
		smk3=0;
	if stopsmoking=4 then
		smk4=1;
	else
		smk4=0;
...
** Crude model between years since stopped smoking (exposure) and oral health (outcome).... ;
proc genmod data=smoking.dummies descending;
	...
	estimate "RR Good vs Poor" stopsmoking 1-1/exp;
run;
/** Stopsmoking not significant... but model convergence.


/*** Check oralh again to see if/how it differs by levels of the exposure?
Chisq not significant, however.... */
proc freq data=smoking.dummies;
	tables oralh*stopsmoking/ chisq;
run;

*** We will just leave the stopsmoking variable as is, for now. Now checking if age could be a potential confounder;
proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") age (ref="12 to 14");
	model oralh=age/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" age 1-1/exp;
run;
...
proc genmod data=smoking.dummies;
	class id;
...
proc genmod data=smoking.dummies2 descending;
	class oralh (param=ref ref="Poor") age_bin (ref="0");
	model oralh=age_bin/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" age_bin 1-1/exp;
run;
...
proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") education(ref="< high school");
	model oralh=education/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" education 1-1/exp;
run;

*** bmi p<.05... keep;

proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") bmi (ref="Underweight");
	model oralh=bmi/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" bmi 1-1/exp;
run;
...
proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") toothbrushing (ref="At least 1x day");
	model oralh=toothbrushing/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" toothbrushing 1-1/exp;
run;

*** dental visits non-significant;

proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") dentalvisits(ref="> 1x a year");
	model oralh=dentalvisits/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" dentalvisits 1-1/exp;
run;

*** alcohol non-significant;

proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") alcohol (ref="< 1x a month");
	model oralh=alcohol/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" alcohol 1-1/exp;
run;

*** income is non-significant;

proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") income (ref="No income, or income loss");
	model oralh=income/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" income 1-1/exp;
run;
...
proc genmod data=smoking.dummies descending;
	class oralh (param=ref ref="Poor") drugs (ref="Used drugs - 12 months");
	model oralh=drugs/ dist=bin link=log lrci;
	estimate "RR Good vs Poor" drugs 1-1/exp;
run;
...
proc genmod data=smoking.dummies descending;
	class oralh(param=ref ref="Poor") drugs(ref="Used drugs - 12 months") 
		BMI(ref="Obese - Class I, II, III") stopsmoking(ref="< 1 year");
